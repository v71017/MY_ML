...
class LabeledCorpus extends Corpus<ObjectHandler<Classified<CharSequence>>> {
	...
	public void visitTrain(ObjectHandler<Classified<CharSequence>> classifier) {
		for (int i = 0; i < N; i++) {
			...
			Classified<CharSequence> labeledInstance = new Classified<CharSequence>(X.get(i), label);
...
public class NaiveBayesClassifier {
	...
	/**
	 * To compensate for the effect of length. In effect, if length(feature) >
	 * N, this pulls the estimate p(c|ws) closer to the category estimate p(c).
	 * If length(feature) < N, it has the opposite effect, and actually
	 ...
	 *            Maximum iteration times
	 */
	public NaiveBayesClassifier(double categoryPrior, double tokenPrior, double lengthNorm, double minTokenCount,
			double minImprovment, int maxIterTimes) {
	 ...
	 * 
	 * @param X
	 *            List of text features separated by white space
	 * @param y
	 *            List of labels, length must be the same with X's
	 */
	public void fit(List<String> X, List<String> y) {
		if (X.size() != y.size()) {
			throw new IllegalArgumentException("Dimension of X and y must" + "be the same");
		}
		...
		TradNaiveBayesClassifier base = new TradNaiveBayesClassifier(CATEGORIES, TOKENIZER, CATEGORY_PRIOR, TOKEN_PRIOR,
				LENGTH_NORM);
		...
		// EM iteration
		Factory<TradNaiveBayesClassifier> factory = new Factory<TradNaiveBayesClassifier>() {
			public TradNaiveBayesClassifier create() {
				TradNaiveBayesClassifier classifier = new TradNaiveBayesClassifier(CATEGORIES, TOKENIZER,
						CATEGORY_PRIOR, TOKEN_PRIOR, LENGTH_NORM);
				return classifier;
			}
		};
		try {
			mClassifier = TradNaiveBayesClassifier.emTrain(base, factory, // new
																			// classifier
																			// factory
																			// while
																			// iteration
					labeledCorpus, unlabeledCorpus, MIN_TOKEN_COUNT, MAX_ITER, MIN_IMPROVMENT, null // reporter
			);
		} catch (IOException e) {
	 ...
	 * 
	 * @param x
	 *            white space separated text features
	...
	public Entry<String, Double> predict(String x) {
		...
		return new AbstractMap.SimpleEntry<String, Double>(bestCategory, log2prob);
	}
